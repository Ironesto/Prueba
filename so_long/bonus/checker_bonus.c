...
/*   Updated: 2024/01/11 18:06:14 by gpaez-ga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strchr("0CEA", data->cpy[y][x]))
		data->cpy[y][x] = '2';
	if (ft_strchr("0CEA", data->cpy[y - 1][x]) && y - 1 >= 0)
		comp_path(data, y - 1, x);
	if (ft_strchr("0CEA", data->cpy[y + 1][x]) && y + 1 < data->h - 1)
		comp_path(data, y + 1, x);
	if (ft_strchr("0CEA", data->cpy[y][x - 1]) && x - 1 >= 0)
		comp_path(data, y, x - 1);
	if (ft_strchr("0CEA", data->cpy[y][x + 1]) && x + 1 < data->w - 1)
		comp_path(data, y, x + 1);
